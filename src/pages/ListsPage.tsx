...
export default function ListsPage() {
  ...
  const handleCreateList = () => {
    if (newListName.trim()) {
      const newList: WishList = { id: Date.now().toString(), name: newListName, isPrivate: true, items: [], createdAt: new Date().toISOString() };
          ...
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4 pt-4">
                ...
                <Button onClick={handleCreateList} className="w-full">{t("lists.createList")}</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-1">
            <div className="bg-card border border-border rounded-lg overflow-hidden">
              <div className="p-3 border-b border-border"><p className="text-sm font-medium text-muted-foreground">{t("lists.yourLists")} ({lists.length})</p></div>
              <div className="divide-y divide-border">
                {lists.map((list) => (
                  <div key={list.id} className={`p-3 cursor-pointer hover:bg-muted/50 transition-colors ${selectedList?.id === list.id ? "bg-muted" : ""}`} onClick={() => setSelectedList(list)}>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        {list.isPrivate ? <Lock className="w-3 h-3 text-muted-foreground" /> : <Globe className="w-3 h-3 text-muted-foreground" />}
                        <DropdownMenu>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{list.items.length} {t("common.items")}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{listProducts.length} {t("common.items")} • {selectedList.isPrivate ? t("common.private") : t("common.public")}</p>
                  </div>
