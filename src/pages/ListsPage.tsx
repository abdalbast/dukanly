  ...
  const handleCreateList = () => {
    if (newListName.trim()) {
      createList.mutate(newListName.trim(), {
        onSuccess: () => { setNewListName(""); setIsCreateDialogOpen(false); },
      });
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1">
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              <div className="p-4 border-b border-border"><p className="text-sm font-medium text-muted-foreground">{t("lists.yourLists")} ({wishlists.length})</p></div>
              <div className="divide-y divide-border">
                {wishlists.map((list) => (
                  <div key={list.id} className={`p-4 cursor-pointer hover:bg-muted/50 transition-colors ${selectedList?.id === list.id ? "bg-muted" : ""}`} onClick={() => setSelectedListId(list.id)}>
            ...
            {selectedList ? (
              <>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <Button variant="outline" className="rounded-full">{t("lists.shareList")}</Button>
                </div>
                {listProducts.length === 0 ? (
                  <div className="bg-card border border-border rounded-xl p-14 text-center">
                    ...
                    <Button asChild className="rounded-full"><Link to="/">{t("common.startShopping")}</Link></Button>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">{listProducts.map((product) => (<ProductCard key={product.id} product={product} />))}</div>
                )}
              </>
            ) : (
              <div className="bg-card border border-border rounded-xl p-14 text-center">
                ...
                <Button onClick={() => setIsCreateDialogOpen(true)} className="rounded-full"><Plus className="w-4 h-4 mr-2 rtl:ml-2 rtl:mr-0" />{t("lists.createFirstList")}</Button>
              </div>
            )}
          </div>
        </div>
